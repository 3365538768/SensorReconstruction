...
def detect_serial_port():
    ...
    if platform.system() == "Windows":
        ...
        # Linux/macOS: 优先查找Arduino设备 (CP2102, FTDI, Arduino等)
        arduino_keywords = ['cp210', 'ftdi', 'arduino', 'ch340', 'ch341']

        # 首先查找已知的Arduino芯片
        for port in ports:
            port_desc = port.description.lower() if port.description else ""
            if any(keyword in port_desc for keyword in arduino_keywords):
                print(f"🎯 检测到Arduino设备: {port.device} ({port.description})")
                return port.device

        # 其次查找带serial的设备
        for port in ports:
            if 'serial' in port.device.lower():
                print(f"🔍 找到串口设备: {port.device} ({port.description})")
                return port.device

        # 最后查找USB或ACM设备（但排除hub设备）
        for port in ports:
            port_desc = port.description.lower() if port.description else ""
            if any(keyword in port.device.lower() for keyword in ['usb', 'acm', 'tty']):
                if 'hub' not in port_desc and 'billboard' not in port_desc:
                    return port.device
...
def serial_reader():
    ...
    print(
        f"📊 Data source: {'Simulation Mode' if SIMULATION_MODE else 'Serial Mode'}")
...
class App:
    def __init__(self, root):
        ...
        control_frame = tk.Frame(
            self.main_paned, width=280, bg="#ffffff", relief=tk.RAISED, bd=3)
        ...
        status_frame = tk.Frame(
            control_frame, bg="#ecf0f1", relief=tk.GROOVE, bd=2)
        ...
        view_frame = tk.Frame(control_frame, bg="#ecf0f1",
    ...
    def setup_3d_plot(self):
        ...
        self.ax.view_init(elev=self.current_elev,
                          azim=self.current_azim)  # Use stored angles
        ...
        if self.bars is None:
            ...
            if self.colorbar is None:
                ...
                self.colorbar = self.fig.colorbar(
                    sm, ax=self.ax, shrink=0.8, aspect=20, label='Normalized Pressure')
        else:
    ...
    def save_frame(self):
        ...
        messagebox.showinfo(
            "Frame Saved", f"Entry {eid} saved successfully (Total: {len(saved_frames)} frames)")

    def export_csv(self):
        if not saved_frames:
            messagebox.showwarning(
                "No Data", "No frames saved! Please save some frames first.")
            return
        ...
        if not path:
            return
        header = ['frame'] + \
            [f'sensor_{r}_{c}' for r in range(ROWS) for c in range(COLS)]
        try:
            ...
            messagebox.showerror(
                "Export Error", f"Error writing CSV file: {e}")
...
def main():
    ...
    print(
        f"   Data Source: {'Simulation' if SIMULATION_MODE else f'Serial Port {SERIAL_PORT}'}")
