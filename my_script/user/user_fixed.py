...
# -*- coding: utf-8 -*-
"""
Fixed Enhanced Dash UI · All Issues Resolved + Drag Selection
Fixes: ID consistency, bidirectional sync, real-time JSON updates, drag selection
...
class StateManager:
    ...
    def get_bbox_json(self) -> str:
        ...
        # Convert all values to Python native types to avoid JSON serialization issues
        output_dict = {
            "bbox": [
                [float(bbox.xmin), float(bbox.ymin), float(bbox.zmin)],
...
class VisualizationEngine:
    ...
    def create_pointcloud_figure(xyz: np.ndarray) -> go.Figure:
        """Create initial point cloud figure with axes and selection support"""
        fig = go.Figure()
        
        # Add point cloud with selection support
        ...
        fig.update_layout(
            scene=dict(
                aspectmode="data",
                dragmode="select"  # Enable 3D selection
            ),
            title="Point Cloud Visualization (Drag to select region)",
            showlegend=True,
            height=600,
...
main_graph = dcc.Graph(
    ...
    config={"displayModeBar": True, "responsive": True},  # Enable modebar for selection
...
# Range sliders and inputs with fixed IDs
x_controls = html.Div([
    html.H6("X Range", className="fs-6 mt-3 mb-1"),
    dbc.Row([
        dbc.Col([
            dbc.Input(
                id="x-min-input",
...
y_controls = html.Div([
    html.H6("Y Range", className="fs-6 mt-3 mb-1"),
    dbc.Row([
        dbc.Col([
            dbc.Input(
                id="y-min-input",
...
z_controls = html.Div([
    html.H6("Z Range", className="fs-6 mt-3 mb-1"),
    dbc.Row([
        dbc.Col([
            dbc.Input(
                id="z-min-input",
...
# Normal vector sliders
theta_slider = html.Div([
    html.H6("Theta (θ): Inclination from Z-axis", className="fs-6 mb-1"),
    dcc.Slider(
        id="theta-slider",
        min=0,
        max=np.pi,
...
phi_slider = html.Div([
    html.H6("Phi (φ): Azimuth in XY-plane", className="fs-6 mt-3 mb-1"),
    dcc.Slider(
        id="phi-slider",
        min=0,
        max=2*np.pi,
...
app.title = "Fixed Enhanced Deformation Prediction UI"

app.layout = dbc.Container([
    # Header
    html.H3("Enhanced Deformation Prediction UI", className="my-3 text-center fs-2"),
    html.P("Drag in 3D view to select points, use sliders for precise control", 
...
def handle_file_upload(contents):
    ...
    try:
        ...
        print(f"DEBUG: {error_msg}")  # 添加控制台调试输出
        
        # 这是错误的做法，get_state()返回的是拷贝，状态不会被保存
        # state_manager.get_state().add_status(error_msg)  
        
        # 需要通过StateManager的实例方法更新状态
        with state_manager._lock:
            state_manager._state.add_status(error_msg)
        
        # 返回一个错误状态的图形，而不是阻止更新
        error_fig = VisualizationEngine.create_empty_figure()
        error_fig.update_layout(title=f"Upload Error: {str(e)}")
        
        # 返回默认值，让用户看到错误信息
        return (
            error_fig,
            0, 1, [0, 1], 0, 1,  # X controls default
            0, 1, [0, 1], 0, 1,  # Y controls default  
            0, 1, [0, 1], 0, 1   # Z controls default
        )


# Drag selection callback
@callback(
...
def handle_drag_selection(selectedData):
    """Handle drag selection in 3D plot to set bounding box"""
    ...
    if len(points) < 2:  # Need at least 2 points to define a bounding box
        raise dash.exceptions.PreventUpdate
    ...
    # Calculate bounding box from selected points
    ...
    state_manager.get_state().add_status(f"Bounding box set from {len(points)} selected points")
...
# Unified bounding box update callback
@callback(
...
def update_bounding_box(x_range, x_min, x_max, y_range, y_min, y_max, 
    ...
    """Unified callback for bounding box updates with perfect bidirectional sync"""
    state = state_manager.get_state()
    if not state.pointcloud.loaded:
        raise dash.exceptions.PreventUpdate
    ...
    new_xmin, new_xmax = state.bbox.xmin, state.bbox.xmax
    new_ymin, new_ymax = state.bbox.ymin, state.bbox.ymax
    new_zmin, new_zmax = state.bbox.zmin, state.bbox.zmax
    
    # Handle X-axis updates
    if trigger and "x-range-slider" in trigger:
        new_xmin, new_xmax = x_range[0], x_range[1]
    elif trigger and ("x-min-input" in trigger or "x-max-input" in trigger):
        new_xmin = x_min if x_min is not None else state.bbox.xmin
        new_xmax = x_max if x_max is not None else state.bbox.xmax
    
    # Handle Y-axis updates
    if trigger and "y-range-slider" in trigger:
        new_ymin, new_ymax = y_range[0], y_range[1]
    elif trigger and ("y-min-input" in trigger or "y-max-input" in trigger):
        new_ymin = y_min if y_min is not None else state.bbox.ymin
        new_ymax = y_max if y_max is not None else state.bbox.ymax
    
    # Handle Z-axis updates
    if trigger and "z-range-slider" in trigger:
        new_zmin, new_zmax = z_range[0], z_range[1]
    elif trigger and ("z-min-input" in trigger or "z-max-input" in trigger):
        new_zmin = z_min if z_min is not None else state.bbox.zmin
        new_zmax = z_max if z_max is not None else state.bbox.zmax
    
    # Update state
    state_manager.update_bbox(
        xmin=new_xmin, xmax=new_xmax,
        ymin=new_ymin, ymax=new_ymax,
        zmin=new_zmin, zmax=new_zmax
    )
    
    # Get updated state and figure
    updated_state = state_manager.get_state()
    fig = VisualizationEngine.update_figure_with_selection(current_figure, updated_state)
    
    return (
        fig,
        [new_xmin, new_xmax], new_xmin, new_xmax,
        [new_ymin, new_ymax], new_ymin, new_ymax,
        [new_zmin, new_zmax], new_zmin, new_zmax
    )
...
# JSON output update callback (FIXED: Listen to all relevant inputs)
@callback(
    ...
    [
        ...
        Input("main-graph", "selectedData")  # Also listen to drag selection
    ],
    prevent_initial_call=False
)
def update_json_output(*args):
    """Update JSON configuration output in real-time"""
...
@callback(
    ...
    [
        ...
        Input("x-range-slider", "value"), Input("y-range-slider", "value"), Input("z-range-slider", "value"),
        Input("theta-slider", "value"), Input("phi-slider", "value"),
        Input("main-graph", "selectedData"),
        Input("file-upload", "contents")  # 添加文件上传监听器
    ],
    prevent_initial_call=True
)
def update_status_log(*args):
...
if __name__ == "__main__":
    print("🚀 Starting Fixed Enhanced Deformation Prediction UI...")
    print("✅ Fixed: ID consistency, bidirectional sync, real-time JSON updates")
    print("🆕 Added: Drag selection in 3D plot")
