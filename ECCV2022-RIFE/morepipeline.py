...
VIEWS = ["A","B"]
TIME_MAP = {"A":0.000000,"B":1.000000}
...
def main(skip_interp):
    ...
    for frame in frames:
        ...
        rot = info_map[frame]["rotation"]
        mat = info_map[frame]["transform_matrix"]

        if skip_interp:
            # 直接整理原始视角帧到 FINAL
            for k, view in enumerate(VIEWS):
                tgt_dir = os.path.join(FINAL_DIR, f"{k:03d}")
            ...
            # 拷贝插帧结果并记录 transforms
            for k, fn in enumerate(outs):
                tgt_dir = os.path.join(FINAL_DIR, f"{k:03d}")
                os.makedirs(tgt_dir, exist_ok=True)
                shutil.copy(os.path.join(out_dir, fn), os.path.join(tgt_dir, frame))
                trans_by_time[k].append({
                    "file_path": f"./{k:03d}/{frame[:-4]}",
                    "rotation": rot,
                    "time": times[k],
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Prepare frames with optional interpolation")
    parser.add_argument("--skip_interp", action="store_true",
                        help="Skip frame interpolation, only organize original frames")
    args = parser.parse_args()
    main(args.skip_interp)
