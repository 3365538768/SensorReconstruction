...
class GaussianModel:
    ...
    def training_setup(self, training_args):
        ...
        # 保存自训练开始以来每个点在各时间步的最大绝对位移，用于后续静态点剪枝
        self.static_disp_threshold = getattr(training_args, "static_disp_threshold", 0.0)
    ...
    def prune(self, max_grad, min_opacity, extent, max_screen_size):
        ...
        # 额外：剪除在所有时间步内位移幅度很小（近乎静态）的点
        if self.static_disp_threshold > 0:
            # deformation_accum 记录了 xyz 维度的最大绝对位移
            disp_max = self._deformation_accum.max(dim=-1).values
            static_mask = disp_max < self.static_disp_threshold
            # 若全部点都满足 static_mask，意味着现在还没足够运动信息——避免删光
            if static_mask.all():
                pass  # 跳过本轮静态剪枝，留待后续再判断
            else:
                prune_mask = torch.logical_or(prune_mask, static_mask)
